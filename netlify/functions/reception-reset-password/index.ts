 ...
 * tenant. Registra el reset en notas_admin (sin guardar la contraseña).
 */
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const { usuario_id } = JSON.parse(event.body || '{}') as { usuario_id?: string };
    ...
    const { data: target, error: targetErr } = await supabaseAdmin
      .from('usuarios')
      .select('id, auth_id, tenant_id, email, notas_admin')
      .eq('id', usuario_id)
      .maybeSingle();
    ...
    // Auditoría (NO se guarda la contraseña).
    const fecha = new Date().toLocaleString('es-MX', { dateStyle: 'short', timeStyle: 'short' });
    const quien = caller.rol === 'admin' ? 'admin' : 'recepción';
    const linea = `[${fecha} · ${quien}] reset de contraseña`;
    await supabaseAdmin
      .from('usuarios')
      .update({ notas_admin: target.notas_admin ? `${target.notas_admin}\n${linea}` : linea })
      .eq('id', target.id);
